/* ************************************************************************************************
                               TX EMG for Stroke Study Application
 ************************************************************************************************
 ...
 ************************************************************************************************
  ...
  The BNO055 IMU continuously senses the position of the lower arm. The goal is to use the sensor
  to measure the angular velocity of the limb segment during the assessment. Additionally the IMU is used
  to measure the orientation of the arm by looking at the x,y,z orientation of the IMU sensor.
  A switch on the electronics package starts the measurement sesson.
  The IMU data is stored continuously to the SD card and also streamed out to a real time dashboard on a PC
  ...
  and so this program stores the raw data to the SD card and also rectifies it to send out using XBee.

  The datastream from A0 is the raw EMG. We are sampling this at about 1kHz.  The program detects at the start the baseline level 
  ...
  The raw EMG is input into an array only during a burst and then when the burst if over this array is read to the SD card.
  
  When a contraction occurs raw EMG data is acquired while it lasts.  Then at the end of the burst the raw emg data is stored.
  The rectified data is transmitted in real time for display on the dashboard along with the IMU information.  We will store the rectified
  and IMU data on the PC and not on the SD card.   This way the raw EMG data is only used as a backup and for future analysis.
...
uint8_t payload_response[1];
Tx16Request tx_response1 = Tx16Request(0x5555, payload_response, sizeof(payload_response));   // Channel 1
Tx16Request tx_response2 = Tx16Request(0x6666, payload_response, sizeof(payload_response));   // Channel 2
...
void setup() {
  ...
  pinMode(8, LOW);
  pinMode(12, OUTPUT);                         // GREEN LARGE LED 
  pinMode(12, LOW);
  pinMode(10,INPUT_PULLUP);
  pinMode(5,INPUT_PULLUP);                    // High = Right LOW = Left
  i = 0;
...
// We are not longer using the SD Card on the Sensor and the Special External Board Stores Data
// Along with the Dashboard that also stores the rectified EMG and other slower sampled parameters.
/*
  ...
  // *******  Give the file a filename incrementing file number as necessary *******************

  strcpy(filename, "/EMG00.CSV");
      for (uint8_t q = 0; q < 100; q++) {           // We can have up to 100 files incremented automatically
        filename[4] = '0' + q/10;
        filename[5] = '0' + q%10;
  ...
  //logfile.println("Everything Checks Out ... Ready!");
*/
...
delay(1000);
 ...
 /* 
  memset(scratch1, 0, sizeof(scratch1));   // Initialize the arrays with zeros
  memset(readings1, 0, sizeof(readings1));
  memset(dataset, 0, sizeof(dataset));
  memset(payload, 0, sizeof(payload));
 */
  /*
  logfile.println("Wearable EMG Data System For Stroke Study");
  logfile.println("Collecting Data");
  logfile.print("Battery Voltage Volts:   "); 
  logfile.println(measuredvbat);
  //logfile.println("Leaving Setup");        // We leave the file open for use in loop where button closes it.
*/
...
// *************************************************************** BASELINEDATA() *******************************************************
// ************************************************** FUNCTION TO SAVE BASELINE DATA TO SD CARD *****************************************/
// **************************************************************************************************************************************

void baselinedata() {
  //Serial.println("In baseline data");
/*  
  logfile.print("Baseline CH3:  ");
  logfile.println(avevalue1);                         // Baseline value
  logfile.print("STDEV CH3:  ");                      // Saving stdev value CH4 - base line
  logfile.println(sdval1);
  // logfile.print("Processing Time:  ");
  // logfile.println(procTime);                       // Gives a measure of sampling rate for raw emg
  logfile.println("End of Baseline Data");
  logfile.println("");
  */  
}
// End of Function
...
// THIS IS THE CALIBRATION APRIL 11 2023

int valuetry = analogRead(0)*0.0018 + 15.216;  // Cal in Newtons
value0= (analogRead(0));
...
//Serial.print("AMG2:");Serial.println(value5);
                ...
                payload_1[ 6] = (average1 >> 8) & 0xff;     // EMG  (H byte) (would normally be EMG)
...
//Serial.print("AMG2:");Serial.println(value5);
                ...
                payload_2[ 12] = (value3 >>   8) & 0xff;   // RAW EMG
                payload_2[ 13] = (value3       ) & 0xff;   // RAW EMG  (L byte)
                payload_2[ 14] = (value4 >>   8) & 0xff;   // AMG (H byte)
